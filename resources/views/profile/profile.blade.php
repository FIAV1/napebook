<div class="d-flex row">
    ...
    <!-- Card User Info -->
    <div id="info-container" class="col-12 col-md-8 order-2 order-md-1">
        <div class="card bg-dark text-white mx-auto mt-5 mt-md-0">
            ...
            <div class="card-header d-flex" id="info-heading">
                ...
                <a class="btn btn-secondary ml-auto rounded-circle" data-toggle="modal" data-target="#edit-profile"><i class="fas fa-pencil-alt"></i></a>
                @endcan
            </div>

            <div id="info" class="collapse show" aria-labelledby="info-heading" data-parent="#info-container">
                <div class="card-body">
    ...
    <!-- Image User -->
    <div class="align-self-center col-12 col-md-4 order-1 order-md-2">
        <img id="profile-image" src="/storage/{{ $user->image_url }}" alt="user image" class="img-fluid img-thumbnail rounded-circle">

        @can('editProfile', $user)
        <form id="edit-profile-image" method="POST" action="{{ route('profile-image-update', $user->id) }}" enctype="multipart/form-data">
            {{ csrf_field() }}
            {{ method_field('PUT') }}
            <label class="btn btn-secondary rounded-circle" for="update-profile-image"><i class="fas fa-pencil-alt"></i></label>
            <input type="file" name="user-image" id="update-profile-image" data-id="{{ $user->id }}" class="form-control" accept=".jpg, .jpeg, .png">
        </form>
        @endcan
